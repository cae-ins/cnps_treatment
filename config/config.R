...
archive_file <- function(filepath) {
  ...
  if (!PROCESSING$archive_replaced_files) {
    return(NULL)
  }
...
MINIO <- list(
  # Connexion
  endpoint = "http://192.168.1.230:31157",
  access_key = Sys.getenv("MINIO_ACCESS_KEY", "minioadmin"),
  secret_key = Sys.getenv("MINIO_SECRET_KEY", "minioadmin"),

  # Bucket
  bucket = "admindataanstat",
  prefix = "CNPS/raw_data/",
