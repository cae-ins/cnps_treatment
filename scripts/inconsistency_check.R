...
# Audit complet des fichiers CNPS avec gestion incrémentale
# ============================================================
...
run_inconsistency_check <- function(input_folder = NULL, 
  ...
  output_file <- file.path(
    output_folder, 
    paste0("audit_fichiers_cnps_", suffix, ".xlsx")
  )
  ...
  # ------------------------------------------------------------
  # IMPORT DES DONNÉES (BASES BRUTES)
  # ------------------------------------------------------------
  ...
  # ------------------------------------------------------------
  # 1. CHECK DOUBLONS LIGNES (BASE BRUTE)
  # ------------------------------------------------------------
  message("Vérification des doublons...")
  
  doublons_lignes <- map2_dfr(
    data_raw,
    file_info$fichier,
    function(df, f) {
      n_tot <- nrow(df)
  ...
  # ------------------------------------------------------------
  # 2. SUPPRESSION DES LIGNES DOUBLONNÉES POUR ANALYSES
  # ------------------------------------------------------------
  ...
  id_var <- REQUIRED_VARS$id[1]  # "ID_INDIV"
  ...
  # ------------------------------------------------------------
  # 8. MATCHING MATRIX ID_INDIV
  # ------------------------------------------------------------
  message("Calcul de la matrice de matching...")
  
  id_list <- map(data_clean, ~ unique(na.omit(.x[[id_var]])))
  sizes   <- map_int(id_list, length)
  n       <- length(id_list)
  
  mat_nb  <- matrix(0, n, n)
  mat_pct <- matrix(0, n, n)
  
  rownames(mat_nb)  <- colnames(mat_nb)  <- file_info$fichier
  rownames(mat_pct) <- colnames(mat_pct) <- file_info$fichier
  
  for (i in seq_len(n)) {
    for (j in seq_len(n)) {
      inter <- length(intersect(id_list[[i]], id_list[[j]]))
      mat_nb[i, j]  <- inter
      mat_pct[i, j] <- round(inter / min(sizes[i], sizes[j]) * 100, 2)
    }
  }
  ...
  addWorksheet(wb, "Matching_ID_NB")
  addWorksheet(wb, "Matching_ID_PCT")
  ...
  writeData(wb, "Matching_ID_NB", as.data.frame(mat_nb), rowNames = TRUE)
  writeData(wb, "Matching_ID_PCT", as.data.frame(mat_pct), rowNames = TRUE)
