...
Replicates the 6-check audit from CNPS_TREATMENT_PROJECT (01_inconsistency_check.R)
adapted to the V2 pipeline (Polars + Parquet).

Checks
------
1. Doublons_lignes     — Duplicate row detection per file
2. Colonnes            — Column consistency across files (vs reference)
3. Types_variables     — Data type mismatches across files
4. Valeurs_manquantes  — Missing value rates per variable per file
5. Outliers_Salaire    — IQR-based outlier detection on SALAIRE_BRUT
6. Unicite_ID          — ID_INDIV uniqueness per file
7. Top_doublons_ID     — Top 5% most duplicated individuals per month
8. Transitions_ID      — Transition matrix: proportion of IDs present from
                          one month to another (row = origin, col = destination)

Output: an Excel workbook with one sheet per check.
"""
...
def _parse_period(filename: str) -> tuple[int, int]:
    """Extract (MOIS, ANNEE) from a filename like '01_2024.parquet'."""
...
def _check_colonnes(data: list[tuple[str, int, int, pl.DataFrame]]) -> pl.DataFrame:
    """2. Column consistency — compare each file against the first (reference)."""
    if not data:
        return pl.DataFrame()
...
def _check_types(data: list[tuple[str, int, int, pl.DataFrame]]) -> pl.DataFrame:
    """3. Data type mismatches across files (vs reference)."""
    if not data:
        return pl.DataFrame()
...
def _check_valeurs_manquantes(data: list[tuple[str, int, int, pl.DataFrame]]) -> pl.DataFrame:
    """4. Missing value count per variable per file."""
    rows = []
    for fname, mois, annee, df in data:
        n_tot = df.height
...
def _check_outliers(data: list[tuple[str, int, int, pl.DataFrame]],
    ...
    """5. IQR-based outlier detection on a salary variable."""
    rows = []
    for fname, mois, annee, df in data:
...
def _check_top_doublons_id(data: list[tuple[str, int, int, pl.DataFrame]],
    ...
    """7. Top most-duplicated individuals per month (top 5% by occurrence count).

    For each file, counts occurrences of each ID, then keeps only those
    in the top ``top_pct`` percentile (i.e. the most repeated IDs).
    """
    frames = []
    for fname, mois, annee, df in data:
        if id_var not in df.columns:
        ...
        counts = (
            ...
            .filter(pl.col("nb_occurrences") > 1)  # only duplicated
        ...
        # Keep top top_pct (at least 1 row)
    ...
    result = pl.concat(frames).select(
        "fichier", "ANNEE", "MOIS", id_var, "nb_occurrences",
    )
    return result
...
def _check_transitions(data: list[tuple[str, int, int, pl.DataFrame]],
                       id_var: str = "ID_INDIV") -> pl.DataFrame:
    """7. Transition matrix — proportion of IDs from month i present in month j.

    Builds an N×N matrix where rows = origin period, columns = destination period.
    Cell (i, j) = |IDs_i ∩ IDs_j| / |IDs_i|  (proportion of i's IDs found in j).
    """
    # Build period labels and ID sets
    periods: list[str] = []
    id_sets: list[set] = []
    ...
    # Build the matrix as a dict of columns
    rows = []
    for i in range(n):
...
def run_audit(
    ...
    *,
    ...
    id_var : str
        Column name for ID uniqueness check.
    ...
    _export_audit_excel(
        ...
        types=df_types,
        valeurs_manquantes=df_missing,
        outliers=df_outliers,
        unicite_id=df_unicite,
        top_doublons_id=df_top_dup,
...
def _write_transition_sheet(
    ...
    """Write the transition matrix sheet with conditional color formatting."""
    ...
    # Color scale formats: from red (low retention) to green (high retention)
    # We create a few bucket formats for the percentage cells
    pct_fmt_low = wb.add_format({
        "num_format": "0.00", "border": 1,
        "bg_color": "#F1948A", "align": "center",   # red — < 25%
    })
    pct_fmt_mid_low = wb.add_format({
        "num_format": "0.00", "border": 1,
        "bg_color": "#F9E79F", "align": "center",   # yellow — 25-50%
    })
    pct_fmt_mid = wb.add_format({
        "num_format": "0.00", "border": 1,
        "bg_color": "#ABEBC6", "align": "center",   # light green — 50-75%
    })
    pct_fmt_high = wb.add_format({
        "num_format": "0.00", "border": 1,
        "bg_color": "#82E0AA", "align": "center",   # green — 75-100%
    })
    pct_fmt_diag = wb.add_format({
        "num_format": "0.00", "border": 1,
        "bg_color": "#5DADE2", "font_color": "#FFFFFF",
        "bold": True, "align": "center",             # blue — diagonal (100%)
    })

    period_cols_start = 2  # columns 0=Periode, 1=Nb_ID, then period columns
    ...
    for ri in range(df.height):
        for ci, col_name in enumerate(cols):
            ...
            if ci == 0:
                ...
                # Percentage cell — pick format by value
                v = float(value) if value is not None else 0.0
                if ci - period_cols_start == ri:
                    fmt = pct_fmt_diag       # diagonal
                elif v >= 75:
                    fmt = pct_fmt_high
                elif v >= 50:
                    fmt = pct_fmt_mid
    ...
    # Column widths
    ws.set_column(0, 0, 14)   # Periode
    ws.set_column(1, 1, 12)   # Nb_ID
    ws.set_column(2, len(cols) - 1, 10)  # period columns

    # Freeze first row + first 2 columns (Periode, Nb_ID)
    ws.freeze_panes(1, 2)
